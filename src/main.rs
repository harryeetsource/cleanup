...
fn cleanup_font_cache(system_root: &str, error_messages: &mut Vec<String>) {
    ...
    let font_cache_cleanup_commands = vec![
        ...
        SystemCommand { program: "del", args: vec!["/f", "/s", "/q", "/a", &font_cache_system_path] },
...
fn disable_insecure_windows_features(error_messages: &mut Vec<String>) {
    ...
    let windows_feature_disable_commands = vec![
        ...
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server", "/v", "fDenyTSConnections", "/t", "REG_DWORD", "/d", "1", "/f"]},
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Control\\Remote Assistance", "/v", "fAllowToGetHelp", "/t", "REG_DWORD", "/d", "0", "/f"]},
        SystemCommand { program: "reg", args: vec!["add", "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer", "/v", "NoDriveTypeAutoRun", "/t", "REG_DWORD", "/d", "255", "/f"]},
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows NT\\DNSClient", "/v", "EnableMulticast", "/t", "REG_DWORD", "/d", "0", "/f"]},
    ];
...
fn enable_credential_guard(error_messages: &mut Vec<String>) {
    ...
    let enable_credential_guard_commands = vec![
       ...
       SystemCommand { program: "bcdedit", args: vec!["/set", "{0cb3b571-2f2e-4343-a879-d86a476d7215}", "device", "path", "\\EFI\\Microsoft\\Boot\\SecConfig.efi"]},
...
fn disable_office_macros(error_messages: &mut Vec<String>) {
    ...
    let macro_disable_commands = vec![
        ...
        SystemCommand { program: "reg", args: vec!["add", "HKEY_CURRENT_USER\\Software\\Microsoft\\Office\\16.0\\Word\\Security", "/v", "VBAWarnings", "/t", "REG_DWORD", "/d", "4", "/f"]},
...
fn enable_windows_defender_realtime_protection(error_messages: &mut Vec<String>){
    ...
    let windows_def_rt_protection = vec![
        ...
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows Defender\\Real-Time Protection", "/v", "DisableOnAccessProtection", "/t", "REG_DWORD", "/d", "0", "/f"]},
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows Defender\\Real-Time Protection", "/v", "DisableScanOnRealtimeEnable", "/t", "REG_DWORD", "/d", "0", "/f"]},
...
fn restrict_lsa_access(error_messages: &mut Vec<String>){
    ...
    let lsa_commands = vec![
        ...
        SystemCommand { program: "reg", args: vec!["add", "HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\Windows\\DeliveryOptimization", "/v", "DODownloadMode", "/t", "REG_DWORD", "/d", "0", "/f"]},
        SystemCommand { program: "reg", args: vec!["add", "HKLM\\SYSTEM\\CurrentControlSet\\Control\\DeviceGuard\\Scenarios\\HypervisorEnforcedCodeIntegrity\\", "/v", "Enabled", "/t", "REG_DWORD", "/d", "1", "/f"]},
